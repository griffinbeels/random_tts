...
pip_env=$dir/tts/bin/pip
python_env=$dir/tts/bin/python3
# update pip
$pip_env install -U pip
# then write to ~/.bashrc and ~/.bash_profile that tts from now on is to activate the
# virtual environment
echo "alias tts='source ~/tts/bin/activate'" >> ~/.bashrc
echo "alias tts='source ~/tts/bin/activate'" >> ~/.bash_profile
source ~/.bashrc
source ~/.bash_profile
